...
#     parser.add_argument("--gt_dir", default="./dataset/TP-Dataset/GroundTruth", help="The root of TP-Dataset ground truth list file")
#     parser.add_argument("--txt_dir", default="./dataset/TP-Dataset/Index/predict.txt", help="The root of predicted file")
#     parser.add_argument("--pred_dir", default="./predict/Unet没扩充的", help="The root of predicted results of testing samples")
...
def main():
    ...
    count = 0
    png_name_list = []
    png_name_list_2 = []  # 新增：用于预测图像的文件名列表
    with open(os.path.join(txt_dir), 'r') as f:
        file_name = [x.strip() for x in f.readlines() if len(x.strip()) > 0]
    for file in file_name:
        count = count + 1
        # 修改这里：只使用文件名部分，不包含路径
        base_name = os.path.basename(file) + ".png"
        png_name_list.append(file + ".png")  # 保持原样用于gt
        png_name_list_2.append(base_name)    # 只使用文件名用于pred
        # The path of logs
    log_path = args.log_path
    ...
    log_file_name = log_path + 'log-' + 'GRFBUNet' + '.log'
     # save print message
    sys.stdout = Logger(log_file_name, stream=sys.stdout)
    compute_mIoU(gt_dir, pred_dir, png_name_list, png_name_list_2, num_classes=2, name_classes=["_background_", "Tactile_paving"])
...
def parse_args():
    ...
    parser.add_argument("--gt_dir", default="./dataset/TP-Dataset/GroundTruth", help="The root of TP-Dataset ground truth list file")
    parser.add_argument("--txt_dir", default="./dataset/TP-Dataset/Index/predict.txt", help="The root of predicted file")
    parser.add_argument("--pred_dir", default="./predict/A+B+C扩充后的+clipseg+csa+long", help="The root of predicted results of testing samples")
    parser.add_argument("--log_path", default="./log", help="log root")
