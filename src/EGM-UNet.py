...
class MCALayer(nn.Module):
    ...
    def forward(self, x):
        ...
        # 自适应融合 - 使用简单的加权平均
        # 这些权重是固定的，不增加参数,这是未扩充的
        # enhanced = 0.4 * x_out + 0.1 * local_range + 0.2 * local_variance + 0.2 * freq_enhanced + 0.1 * shuffled
        # output = identity + enhanced+x_out
...
class SoftPooling2D(torch.nn.Module):
    ...
    def forward(self, x):
        # 全局软池化：对整个空间维度进行软池化，输出形状为 [B, C, 1, 1]
        b, c, h, w = x.size()
        ...
        # 计算加权和
        weighted_sum = torch.sum(x_exp * x_flat, dim=2, keepdim=True)
        
        # 计算软池化结果
        soft_pool = weighted_sum / x_exp_sum
        
        # 重塑为 [B, C, 1, 1]
        return soft_pool.view(b, c, 1, 1)
class StdPool(nn.Module):
    def __init__(self):
        super(StdPool, self).__init__()
    def forward(self, x):
    ...
    def __init__(self, k_size, pool_types=['avg', 'std']):
        ...
        for pool_type in pool_types:
            if pool_type == 'avg':
                ...
                self.pools.append(StdPool())
            # elif pool_type == 'soft': #这也是未扩充的
            #     self.pools.append(SoftPooling2D())
            else:
                raise NotImplementedError
        self.conv = nn.Conv2d(1, 1, kernel_size=(1, k_size), stride=1, padding=(0, (k_size - 1) // 2), bias=False)
        self.sigmoid = nn.Sigmoid()
