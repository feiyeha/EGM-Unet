...
class CLIP(nn.Module):
    ...
    #     if load_from_clip == False:
    #         self.positional_embedding = nn.Parameter(torch.empty(248, transformer_width))
    #         # 新增：用原始 77 长度位置嵌入插值得到 248 长度的 positional_embedding_res
    #         self.positional_embedding_res = nn.Parameter(
    #             torch.nn.functional.interpolate(
    #                 self.positional_embedding.unsqueeze(0).unsqueeze(0),  # [1,1,77,512]
    #                 size=(248, transformer_width),  # 插值到 248 长度
...
#         return x[torch.arange(x.shape[0]), text.argmax(dim=-1)] @ self.text_projection
    ...
    def forward(self, image, text_long,text_short,rank):
        image_features_long = self.encode_image(image,csa=True)
